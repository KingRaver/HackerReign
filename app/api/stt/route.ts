...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const whisperCommand = `"${whisperBin}" "${tempAudioPath}" --model small --task transcribe --output_format json --output_dir "${tempDir}"`;

    console.log('[STT] Starting Whisper transcription...');
    const startTime = Date.now();

    try {
      execSync(whisperCommand, {
        timeout: 60000, // 60 second timeout for audio processing
        stdio: 'pipe', // Suppress whisper's stdout/stderr
      });
